...
<body>
        ...
        <div class="formbody">
            ...
            <ul class="forminfo">
                ...
                <li>
                    ...
                    <?php if(empty($data["duty_id"])): ?><option value="">选择所属职务</option>
                    <?php else: ?>
                        <option value="<?php echo ($data['duty_id']); ?>"><?php echo ($data['duty_data']['duty_name']); ?></option><?php endif; ?>
                    
                    <?php if(is_array($duty_data)): $i = 0; $__LIST__ = $duty_data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["id"]); ?>"><?php echo ($vo["duty_name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                    </select>
                </li>
                 <li>
                    <label>用户类型</label>
                    <select name="user_type" id="" style="background-color: #96cef3;margin-top:10px;">
                    <?php if($data['user_type'] == 0): ?><option value="0">普通用户</option>
                    <?php else: ?>
                ...
                <li><label>用户姓名</label><input name="username" type="text" value="<?php echo ($data['username']); ?>" class="dfinput" /></li>
                <li><label>登录账户</label><input name="account" type="text" value="<?php echo ($data['account']); ?>" class="dfinput" /></li>
    ...
    <script type="text/javascript">
        $(".btn").click(function(){
            $.ajax({
                type : "POST",
                url : "<?php echo u('Index/user_edit_do');?>",
